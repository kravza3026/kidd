@php
    // Shared admin navigation for both the desktop sidebar (collapsible) and the mobile
    // drawer (always expanded). Items only render when their route exists AND the user has
    // the permission AND the Pennant module is active, so the menu grows as modules ship.
    $collapsible = $collapsible ?? true;

    $groups = [
        'Catalog' => [
            ['route' => 'admin.products.index', 'label' => __('Products'), 'permission' => 'product.viewAny', 'feature' => 'admin.product', 'icon' => 'M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6Zm9.75 0A2.25 2.25 0 0 1 15.75 3.75H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6Zm-9.75 9.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25Zm9.75 0a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25Z'],
            ['route' => 'admin.categories.index', 'label' => __('Categories'), 'permission' => 'category.viewAny', 'feature' => 'admin.category', 'icon' => 'M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44Z'],
            ['route' => 'admin.tags.index', 'label' => __('Tags'), 'permission' => 'tag.viewAny', 'feature' => 'admin.tag', 'icon' => 'M9.568 3H5.25A2.25 2.25 0 0 0 3 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 0 0 5.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 0 0 9.568 3Z'],
        ],
        // Catalog attributes — rendered as a collapsible group (see $collapsibleGroups).
        'Attributes' => [
            ['route' => 'admin.brands.index', 'label' => __('Brands'), 'permission' => 'brand.viewAny', 'feature' => 'admin.brand', 'icon' => 'M9.568 3H5.25A2.25 2.25 0 0 0 3 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 0 0 5.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 0 0 9.568 3Z M6 6h.008v.008H6V6Z'],
            ['route' => 'admin.colors.index', 'label' => __('Colors'), 'permission' => 'color.viewAny', 'feature' => 'admin.color', 'icon' => 'M4.098 19.902a3.75 3.75 0 0 0 5.304 0l6.401-6.402M6.75 21A3.75 3.75 0 0 1 3 17.25V4.125C3 3.504 3.504 3 4.125 3h5.25c.621 0 1.125.504 1.125 1.125v4.072M6.75 21a3.75 3.75 0 0 0 3.75-3.75V8.197M6.75 21h13.125c.621 0 1.125-.504 1.125-1.125v-5.25c0-.621-.504-1.125-1.125-1.125h-4.072M10.5 8.197l2.88-2.88c.438-.439 1.15-.439 1.59 0l3.712 3.713c.44.44.44 1.152 0 1.59l-2.879 2.88M6.75 17.25h.008v.008H6.75v-.008Z'],
            ['route' => 'admin.sizes.index', 'label' => __('Sizes'), 'permission' => 'size.viewAny', 'feature' => 'admin.size', 'icon' => 'M21.75 17.25v-.228a4.5 4.5 0 0 0-.12-1.03l-2.268-9.64a3.375 3.375 0 0 0-3.285-2.602H7.923a3.375 3.375 0 0 0-3.285 2.602l-2.268 9.64a4.5 4.5 0 0 0-.12 1.03v.228m19.5 0a3 3 0 0 1-3 3H5.25a3 3 0 0 1-3-3m19.5 0a3 3 0 0 0-3-3H5.25a3 3 0 0 0-3 3m16.5-9.75h-3.375M8.25 7.5H4.875'],
            ['route' => 'admin.genders.index', 'label' => __('Genders'), 'permission' => 'gender.viewAny', 'feature' => 'admin.gender', 'icon' => 'M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z'],
            ['route' => 'admin.seasons.index', 'label' => __('Seasons'), 'permission' => 'season.viewAny', 'feature' => 'admin.season', 'icon' => 'M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z'],
            ['route' => 'admin.fabrics.index', 'label' => __('Fabrics'), 'permission' => 'fabric.viewAny', 'feature' => 'admin.fabric', 'icon' => 'M3.75 7.5l16.5 0M3.75 12l16.5 0M3.75 16.5l16.5 0'],
            ['route' => 'admin.care-instructions.index', 'label' => __('Care instructions'), 'permission' => 'careInstruction.viewAny', 'feature' => 'admin.careInstruction', 'icon' => 'M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z'],
        ],
        'Sales' => [
            ['route' => 'admin.orders.index', 'label' => __('Orders'), 'permission' => 'order.viewAny', 'feature' => 'admin.order', 'icon' => 'M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z'],
            ['route' => 'admin.customers.index', 'label' => __('Customers'), 'permission' => 'customer.viewAny', 'feature' => 'admin.customer', 'icon' => 'M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z'],
        ],
        'Inventory' => [
            ['route' => 'admin.inventory.index', 'label' => __('Inventory'), 'permission' => 'inventory.viewAny', 'feature' => 'admin.inventory', 'icon' => 'M20.25 7.5l-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z'],
            ['route' => 'admin.warehouses.index', 'label' => __('Warehouses'), 'permission' => 'warehouse.viewAny', 'feature' => 'admin.warehouse', 'icon' => 'M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M6.75 21v-3.375c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21'],
        ],
        'Content' => [
            ['route' => 'admin.vacancies.index', 'label' => __('Vacancies'), 'permission' => 'vacancy.viewAny', 'feature' => 'admin.vacancy', 'icon' => 'M20.25 14.15v4.25c0 1.094-.787 2.036-1.872 2.18-2.087.277-4.216.42-6.378.42s-4.291-.143-6.378-.42c-1.085-.144-1.872-1.086-1.872-2.18v-4.25m16.5 0a2.18 2.18 0 0 0 .75-1.661V8.706c0-1.081-.768-2.015-1.837-2.175a48.114 48.114 0 0 0-3.413-.387m4.5 8.006c-.194.165-.42.295-.673.38A23.978 23.978 0 0 1 12 15.75c-2.648 0-5.195-.429-7.577-1.22a2.016 2.016 0 0 1-.673-.38m0 0A2.18 2.18 0 0 1 3 12.489V8.706c0-1.081.768-2.015 1.837-2.175a48.111 48.111 0 0 1 3.413-.387m7.5 0V5.25A2.25 2.25 0 0 0 13.5 3h-3a2.25 2.25 0 0 0-2.25 2.25v.894m7.5 0a48.667 48.667 0 0 0-7.5 0M12 12.75h.008v.008H12v-.008Z'],
            ['route' => 'admin.vacancy-applications.index', 'label' => __('Applications'), 'permission' => 'vacancyApplication.viewAny', 'feature' => 'admin.vacancyApplication', 'icon' => 'M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z'],
            ['route' => 'admin.contact-inquiries.index', 'label' => __('Inquiries'), 'permission' => 'contactInquire.viewAny', 'feature' => 'admin.contactInquire', 'icon' => 'M2.25 12.76c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.184-4.183a1.14 1.14 0 0 1 .778-.332 48.294 48.294 0 0 0 5.83-.498c1.585-.233 2.708-1.626 2.708-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z'],
            ['route' => 'admin.locations.index', 'label' => __('Locations'), 'permission' => 'location.viewAny', 'feature' => 'admin.location', 'icon' => 'M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z'],
            ['route' => 'admin.companies.index', 'label' => __('Companies'), 'permission' => 'company.viewAny', 'feature' => 'admin.company', 'icon' => 'M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21'],
        ],
        'Platform' => [
            ['route' => 'admin.users.index', 'label' => __('Users & staff'), 'permission' => 'user.viewAny', 'feature' => 'admin.user', 'icon' => 'M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z'],
            ['route' => 'admin.roles.index', 'label' => __('Roles & permissions'), 'permission' => 'role.viewAny', 'feature' => 'admin.role', 'icon' => 'M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z'],
            ['route' => 'admin.settings.edit', 'label' => __('Settings'), 'permission' => 'setting.viewAny', 'feature' => 'admin.setting', 'icon' => 'M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z'],
            ['route' => 'admin.audit.index', 'label' => __('Audit log'), 'permission' => 'audit.viewAny', 'feature' => 'admin.audit', 'icon' => 'M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z'],
            ['route' => 'admin.design.index', 'label' => __('Design system'), 'permission' => null, 'feature' => null, 'icon' => 'M9.53 16.122a3 3 0 0 0-5.78 1.128 2.25 2.25 0 0 1-2.4 2.245 4.5 4.5 0 0 0 8.4-2.245c0-.399-.078-.78-.22-1.128Zm0 0a15.998 15.998 0 0 0 3.388-1.62m-5.043-.025a15.994 15.994 0 0 1 1.622-3.395m3.42 3.42a15.995 15.995 0 0 0 4.764-4.648l3.876-5.814a1.151 1.151 0 0 0-1.597-1.597L14.146 6.32a15.996 15.996 0 0 0-4.649 4.763m3.42 3.42a6.776 6.776 0 0 0-3.42-3.42'],
        ],
    ];

    $collapsibleGroups = ['Attributes'];
@endphp

<nav class="flex flex-1 flex-col gap-y-4">
    <x-admin.nav-item route="admin.home" :label="__('Dashboard')" :collapsible="$collapsible" icon="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />

    @foreach ($groups as $heading => $items)
        @php($visibleItems = collect($items)->filter(fn ($i) => \Illuminate\Support\Facades\Route::has($i['route'])
            && (! $i['permission'] || auth()->user()?->can($i['permission']))
            && (! ($i['feature'] ?? null) || \Laravel\Pennant\Feature::active($i['feature']))))
        @if ($visibleItems->isNotEmpty())
            @php($collapsibleGroup = $collapsible && in_array($heading, $collapsibleGroups, true))
            @php($groupActive = collect($items)->contains(fn ($i) => request()->routeIs(\Illuminate\Support\Str::beforeLast($i['route'], '.').'.*')))
            <div @if ($collapsibleGroup) x-data="{ open: @js($groupActive) }" @endif>
                @if ($collapsibleGroup)
                    <button type="button" x-show="!collapsed" @click="open = !open"
                        class="flex w-full items-center justify-between px-2.5 pb-1 text-[10px] font-semibold tracking-wider text-ink-subtle uppercase transition-colors hover:text-ink-muted">
                        <span>{{ __($heading) }}</span>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="size-3 transition-transform" :class="open && 'rotate-90'">
                            <path d="m9 5 7 7-7 7" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                    </button>
                @else
                    <p @if ($collapsible) x-show="!collapsed" @endif class="px-2.5 pb-1 text-[10px] font-semibold tracking-wider text-ink-subtle uppercase">{{ __($heading) }}</p>
                @endif
                <div class="space-y-0.5" @if ($collapsibleGroup) x-show="open || collapsed" x-collapse @endif>
                    @foreach ($items as $item)
                        <x-admin.nav-item
                            :route="$item['route']"
                            :label="$item['label']"
                            :icon="$item['icon']"
                            :permission="$item['permission']"
                            :feature="$item['feature'] ?? null"
                            :collapsible="$collapsible"
                        />
                    @endforeach
                </div>
            </div>
        @endif
    @endforeach
</nav>
