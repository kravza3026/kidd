...
@section('checkout-form')
        ...
        <div class="col-span-15 text-2xl font-bold">
            ...
            <p class="text-base font-normal opacity-60">
                ...
                <span class="flex items-center gap-x-4 leading-8 font-medium">
                    ...
                    <span class="border-r border-light-border h-4"></span>
    ...
    <form x-data="{ method: 'card' }" action="{{ route('checkout.process.payment') }}" method="POST" class="space-y-6">
        @csrf
        <div  class="grid grid-cols-17 items-center gap-x-2 space-y-6">
        ...
        <div class="grid grid-cols-17">
            <div class="col-span-16 col-start-2">
                <div x-data="{
                            checked: false,
                            postalCode: '{{ old('shipping_postal_code', $checkoutData['shipping_postal_code'] ?? '') }}',
                            shippingBuilding: '{{ old('shipping_building', $checkoutData['shipping_building'] ?? '') }}',
                            shippingStreet: '{{ old('shipping_street_name', $checkoutData['shipping_street_name'] ?? '') }}'
                            }" id="card-details" class="space-y-4" x-show="method === 'card'">
                    <div class="border-light-border space-y-4 rounded-2xl border">
                        <div
                            class="bg-light-orange grid grid-cols-12 items-center justify-between rounded-2xl p-4">
                            <h2 class="col-span-4 text-base font-bold">
                                Billing address
                            </h2>
                            <div class="col-span-8">

                                <div class="relative w-full flex items-center justify-end gap-x-2">
                                    ...
                                    <div class="relative w-1/2">
                                       ...
                                       <span class="flex items-center gap-x-2">
                                           ...
                                           <span x-text="checked ? shippingStreet : ' {{ __('checkout.shipping.form.saved_addresses_placeholder') }}'" id="selected-option">
                                                ...
                                                @foreach (auth()->user()->addresses as $address)
                                                    ...
                                                    >
                                                        <input
                                                            ...
                                                            value="{{ $address->label }}"
                                                            x-bind:selected="checked && '{{ old('saved_address', $checkoutData['saved_address'] ?? '') == $address->label ? 'true' : 'false' }}'"
                                                        />
                                                        <label
                            ...
                            <div class="col-span-6 mt-3">
                                ...
                                >
                                    ...
                                    @foreach ($regions as $region)
                                        <option
                                            value="{{ $region->id }}"
                                            x-bind:selected="checked && '{{ old('shipping_region', $checkoutData['shipping_region'] ?? '') }}' == '{{ $region->id }}'"
                                        >
                                            {{ $region->name }}
                                        </option>
                                    @endforeach
                            ...
                            <div class="col-span-6 mt-3">
                                ...
                                >
                                    ...
                                    @foreach ($regions->where('id', '=', $checkoutData['shipping_region'] ?? '9')->first()?->cities as $city)
                                        <option
                                            value="{{ $city->id }}"
                                            x-bind:selected="checked && '{{ old('shipping_city', $checkoutData['shipping_city'] ?? '') }}' == '{{ $city->id }}'"
                                        >
                                            {{ $city->name }}
                                        </option>
                                    @endforeach
                ...
                <div id="transfer-details" class="space-y-4" x-show="method === 'transfer'">
                    <h2 class="text-lg font-bold">Transfer</h2>
                </div>
                <div id="online-details" class="space-y-4" x-show="method === 'online'">
                    <h2 class="text-lg font-bold">Online</h2>
                </div>
                <div id="terminal-details" class="space-y-4" x-show="method === 'terminal'">
