...
<div class="cursor-pointer group p-1 w-1/4">
    <div class="bg-card-bg group-hover:bg-white border border-transparent group-hover:border-black/10 duration-700 transition-all ease-in-out rounded-xl py-4 px-2 relative overflow-hidden">
        <div class="relative flex">
            @if($discount || $isNew)
                <div class="absolute top-2 left-2 flex items-center gap-2  ">
                    @if ($discount)
                        <div class=" bg-danger text-white text-[12px] font-semibold rounded-full px-3 py-1">
                            {{ $discount }}
                        </div>
                    @endif
                    @if ($isNew)
                        <div class=" bg-olive text-white text-[12px] font-semibold rounded-full px-3 py-1">
                            NEW
                        </div>
                    @endif
                </div>
            ...
            @if ($genderIcon || $ageLabel)
                <div class="absolute top-2 right-2 flex items-center gap-1  bg-opacity-90 rounded-full px-2 py-1 text-xs">
                    @if ($genderIcon)
        ...
        <div class="absolute bg-white p-2 border border-black/10 rounded-full right-4 bottom-[-20%] group-hover:bottom-4  duration-500 transition-all ease-in-out">
            <img class="opacity-0  group-hover:opacity-100 duration-1000"  src="{{ asset('assets/images/icons/add_fav.svg') }}" alt="">
        </div>
    </div>

    <div class="text-start px-4 mt-4">
        <p class="text-base text-charcoal">{{ $title }}</p>
