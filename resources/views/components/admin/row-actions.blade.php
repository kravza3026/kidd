@props([
    ...
    'deleteId' => null,
    'deleteConfirm' => null,
    // Explicit gates take precedence; otherwise the model's policy is consulted.
    'canView' => null,
    'canEdit' => null,
    'canDelete' => null,
])
...
@php
    $showView = $viewUrl && ($canView ?? ($model && auth()->user()?->can('view', $model)));
    $showEdit = $editUrl && ($canEdit ?? ($model && auth()->user()?->can('update', $model)));
    $showDelete = $deleteId !== null && ($canDelete ?? ($model && auth()->user()?->can('delete', $model)));
@endphp

{{-- Compact icon actions for a table row. Extra actions can be passed as the slot
     (rendered first), each using the .admin-icon-btn class. --}}
<div class="flex items-center justify-end gap-0.5">
    {{ $slot }}

    @if ($showView)
        <a href="{{ $viewUrl }}" wire:navigate title="{{ __('View') }}" aria-label="{{ __('View') }}" class="admin-icon-btn">
