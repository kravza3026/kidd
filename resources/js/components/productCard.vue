    ...
    <div  class="cursor-pointer relative z-0 group p-1 hover:z-10">
        ...
        >
            <div class="relative flex">
                <div
                    v-if="product.is_new || product.has_discount"
                ...
                <div
                    class="absolute top-1 right-1 flex sm:hidden xl:flex items-center gap-1 bg-opacity-90 rounded-full px-2 py-1 text-xs"
                >
        ...
        const showAlert = (productName ,productId) => {
            ...
            Swal.fire({
                ...
                <div class="px-4" style="flex: 1;">
                    <strong class="text-[14px]">${productName}</strong><br>
                    <small class="text-[12px] opacity-60">${message}</small>
                </div>
                <a class="flex items-center gap-x-2 bg-light-orange/20 text-olive rounded-full font-bold h-full px-4 py-3" href="/favorites" >
                    Favorites
                    <svg
                v-if="withArrow"
                width="14"
